...
k.value="";h.innerHTML="";let a=0;for(var b=0;b<e.localStorage.length;b++){const d=e.localStorage.key(b),f=y(d);if("undefined"===typeof!f.full)return;const {full:g,text:l,type:m,url:n}=f;d&&(a++,F(n,m,l,g))}0===a&&(h.innerHTML="No links added...");h.appendChild(z);b=e.localStorage.length;A.innerHTML=`${a}/${a<b?a:b}`}function B(){h.innerHTML="";let a=0;for(let d=0;d<e.localStorage.length;d++){var b=e.localStorage.key(d);b=y(b);if("undefined"===typeof!b.full)return;const {full:f,text:g,type:l,url:m}=
b;"undefined"!==typeof f&&-1<f.indexOf("http")&&l.concat(g.split(),m.split(),f.split()).map(n=>!!n.toLowerCase().includes(k.value.toLowerCase())).includes(!0)&&(a++,F(m,l,g,f))}h.appendChild(z);A.innerHTML=`${a}/${e.localStorage.length}`;0===a&&(h.innerHTML="No results...")}function y(a){try{return JSON.parse(e.localStorage.getItem(a))}catch(b){return console.error("Error parsing JSON:",b),null}}function F(a,b,d,f){const g=c.createElement("a"),l=c.createElement("span"),m=c.createElement("span"),n=
c.createElement("span"),r=c.createElement("span");r.className="close";r.innerHTML="\u00d7";r.onclick=()=>{e.localStorage.removeItem(g.href);B();return!1};g.setAttribute("href",f);g.setAttribute("target","_blank");l.textContent=b;n.textContent=a;m.textContent=d;n.className="urls";g.appendChild(n);g.appendChild(m);g.appendChild(l);g.appendChild(r);z.appendChild(g)}const u=c.getElementById("check-boxes"),C=c.getElementById("select"),J=c.getElementById("choises"),L=c.getElementById("create-link"),h=c.getElementById("outpus"),
M=c.getElementById("exit"),N=c.getElementById("exit1"),O=c.getElementById("open"),K=c.getElementById("aditions"),k=c.getElementById("site-search"),P=c.getElementById("copy-links"),Q=c.getElementById("export-json"),t=c.getElementById("export-div"),G=c.getElementById("links"),A=c.getElementById("founded"),z=c.createDocumentFragment(),D=c.getElementsByName("check")[0],H=c.getElementById("filter");let w=!1;D.checked=!1;H.checked=!1;D.addEventListener("input",()=>D.checked?h.classList.add("checks"):h.classList.remove("checks"));
